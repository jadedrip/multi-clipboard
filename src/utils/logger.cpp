...
void Logger::initialize() {
  ...
  // 日志目录：可执行文件所在目录下的 logs 子目录（便携式，日志跟随程序位置）
  // 注意：initialize() 必须在 QApplication 创建之后调用（applicationDirPath
  // 依赖应用实例）
...
QString Logger::errorLogPath() const { return m_errorLogPath; }
...
void Logger::writeToFile(const QString &level, const QString &line,
                         bool isError) {
  rotateIfNeeded();
...
void Logger::rotateIfNeeded() {
  ...
  const QString oldest =
      QString("%1.%2").arg(m_logFilePath).arg(kMaxBackupCount);
  QFile::remove(oldest);
